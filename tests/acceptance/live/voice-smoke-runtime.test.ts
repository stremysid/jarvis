...
const outboundNoAnswerEvidence = Object.freeze({
  schemaVersion: "1.4",
  generatorVersion: "0.1.0",
  status: "passed",
...
describe("injected live voice-smoke driver", () => {
  ...
  it("injects and verifies an owner call that ran with no authentication prompt through the scenario and enrolled-operator adapters", async () => {
    const observed: unknown[] = [];
    const driver = createVoiceSmokeDriver({
      preflight: async (scenario) => {
        observed.push(["preflight", scenario]);
      ...
      execute: async (request) => {
        ...
        return { schemaVersion: "1.0", scenario: "outbound-no-answer", correlationId: CORRELATION_ID };
      },
      queryEvidence: async (request) => {
        observed.push(["query", request]);
        return outboundNoAnswerEvidence;
      },
    });

    await expect(driver.run("outbound-no-answer")).resolves.toEqual(outboundNoAnswerEvidence);
    expect(observed).toEqual([
      ["preflight", "outbound-no-answer"],
      ["execute", { scenario: "outbound-no-answer", deployedCommitSha: COMMIT_SHA }],
      ["query", {
        scenario: "outbound-no-answer",
        deployedCommitSha: COMMIT_SHA,
        correlationId: CORRELATION_ID,
...
describe("local voice-smoke evidence store", () => {
  it("reports whether a fixed final evidence record is already retained", async () => {
    ...
    await expect(store.exists("failure-callbacks.json")).resolves.toBe(false);
    await expect(store.exists("outbound-no-answer.json")).resolves.toBe(false);
