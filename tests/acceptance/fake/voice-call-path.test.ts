...
describe("fake voice call path", () => {
  ...
  it("admits the owner without a PIN and preserves two turns across an interruption", async () => {
    ...
    try {
      ...
      // An ordinary owner call is asked for nothing: the setup frame alone
      // activates it, so no prompt frame precedes the first request.
      expect(call.frames().map((frame) => frame.token)).toEqual([]);
  ...
  it("answers an owner outbound call without a PIN and ends the active session on a signed terminal callback", async () => {
    ...
    try {
      ...
      expect(call.frames().map((frame) => frame.token).join("|")).toBe(
        "Jarvis called for Sid. No private message was left.",
      );
