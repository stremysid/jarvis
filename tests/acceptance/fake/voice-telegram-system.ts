...
export async function createFakeTelegramCallingSystem() {
  const base = await createFakeCallingSystem();
  await env.DB.batch([
    ...
    env.DB.prepare(`INSERT INTO channel_identities (identity_id, principal_id, channel, provider_subject, status, verified_at, created_at)
      VALUES ('identity:telegram-owner', 'principal:owner', 'telegram', '12345', 'active', ?, ?)`).bind(NOW, NOW),
    env.DB.prepare(`INSERT INTO channel_identities (identity_id, principal_id, channel, provider_subject, status, verified_at, created_at)
