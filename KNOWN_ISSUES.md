...
| Generic redaction has syntactic limits and deliberate false positives. | The [redaction-gap fixtures and evidence](docs/reviews/2026-09-24-redaction-gaps.md) cover contextual assignments and specified phone formats. Bare four-digit numbers intentionally survive; unlabelled passphrases and complete spoken-word PIN sequences are not covered. Unquoted multiword passphrases end at sentence/list punctuation or a line break; quote them to cover interior sentence punctuation. An explicit assignment such as `pin is on` redacts `on`, and `code is` can redact an ordinary identifier. Bare course codes, dates, times and tested ordinary numbers survive. Telegram holds a line containing potential credential context through EOF, which can delay its suffix; voice keeps sentence release. This branch has not been deployed. |
