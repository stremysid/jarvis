...
The only deployment evidence used is [STATE](docs/STATE.md#production):
**as of source `352991e`**, observed 2026-09-21 at 23:20 UTC, Worker upload
`78cb6e98-7814-4be7-82fb-a795a7e4d0a7`, active version
`64a184ce-4408-4962-b973-9ec3b6f48c9c`, D1 through `0038`.
...
| Late passphrase fragments are not universally suppressed. | `#guardOwnerRepeat` in [call-session-do.ts](apps/cloud-gateway/src/voice/call-session-do.ts) returns short fragments as ordinary speech when status is `available` after the fragment window. The `spent` path was corrected by `c58463b`; the old blanket description of all later statuses was inaccurate. |
...
| Live release acceptance is incomplete. | [STATE](docs/STATE.md#production) records inbound passphrase activity as of `352991e`, so “production trusts only the phone number” is obsolete. It records no outbound call and no completed release gate. Answering-machine behavior and attended smoke remain unverified; the passphrase code is not itself live certification. |
...
| Remaining limit | Evidence and boundary |
|---|---|
| External watchdog coverage is unverified. | [health.ts](apps/watchdog/src/health.ts) needs an outside poller to notice total cron failure. None is provisioned by this code; STATE records the remaining external-watchdog gap. Do not infer current account settings from repository absence. |
...
audit. The missing incident report and the task's PC restrictions override
historical suggestions to rerun such probes.
