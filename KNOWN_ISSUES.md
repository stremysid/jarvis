...
| An unreferenced paraphrase can survive previous-reply forgetting. | Previous-reply filtering removes a reply when its durable item ids include a forgotten item, its paired owner turn is suppressed, or its text exactly restates the forgotten current version. A paraphrase with no committed/cited item id has the same lexical limit as canonical retrieval and can remain visible. Widening that judgment matcher is deferred rather than adding another rule in code. |
