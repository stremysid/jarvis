...
| Late passphrase fragments are not universally suppressed. | `#guardOwnerRepeat` in [call-session-do.ts](apps/cloud-gateway/src/voice/call-session-do.ts) returns short fragments as ordinary speech when status is `available` after the fragment window. The `spent` path was corrected by `c58463b` (#137), **not in the recorded deploy `352991e`**; the old blanket description of all later statuses was inaccurate for the audit base. |
...
| Live release acceptance is incomplete. | [STATE](docs/STATE.md) records six inbound owner calls (1 completed, 3 rejected, 1 failed, 1 enrollment), no outbound call and no completed release gate. The recorded deploy `352991e` includes the owner step-up implementation `8120d44`, and STATE records D1 at migration `0038`; these replace the old phone-number-only implementation claim without treating call counts as passphrase acceptance evidence. Answering-machine behavior and attended smoke remain unverified. |
