...
import { canonicalize, compareOrdinal, loadCanonicalJsonFile, sha256Hex } from "./canonical-json.mjs";
...
const SBOM_SHA256 = "dd86883fe52598cb021bcc63c14dea4001186fced460d8af38d48bb0ef16dc66";
const SBOM_FILE_SHA256 = "f447f5df60ef95a21a8f7c465d7f151d50acc97807322e94916f33a2c8810372";
...
const REVIEWED_RUNTIME_PACKAGE = Object.freeze({
  devDependencies: {
    "@iarna/toml": "2.2.5",
    ajv: "8.17.1",
  },
...
function record(value, keys, label) {
  ...
  const actual = Object.getOwnPropertyNames(value).sort(compareOrdinal);
...
function array(value, label) {
  ...
  if (keys.some((key) => typeof key === "symbol")) fail(`${label} array must not contain symbol or extra fields`);
  const lengthDescriptor = descriptors.length;
  if (lengthDescriptor === undefined || !("value" in lengthDescriptor) || lengthDescriptor.enumerable || !Number.isInteger(lengthDescriptor.value) || lengthDescriptor.value < 0 || lengthDescriptor.value > 0xffffffff) fail(`${label} array must have an exact length data field`);
  const length = lengthDescriptor.value;
  if (keys.length !== length + 1) fail(`${label} array must not be sparse or contain extra fields`);
  const values = new Array(length);
  for (let index = 0; index < length; index += 1) {
    const descriptor = descriptors[String(index)];
    if (descriptor === undefined || !("value" in descriptor) || !descriptor.enumerable) fail(`${label} array must be dense enumerable data fields without accessors`);
...
function checkRuntimePackage(runtimePackage) {
  compareCanonical(runtimePackage, REVIEWED_RUNTIME_PACKAGE, "runtime package");
}
...
async function expectedProvenance(source, artifacts, patches) {
  return [
    ...
    ...Object.entries(source.licenses.files).sort(([left], [right]) => compareOrdinal(left, right)).map(([path, digest]) => provenanceComponent(`notice-${path.split("/").at(-1)}`, "pinned", [property("jarvis:path", path), property("jarvis:sha256", digest)])),
  ];
}
...
export async function validateSbomIntegrity({ source, artifacts, patches, sbom }) {
  ...
  const serialized = new TextDecoder("utf-8", { fatal: true }).decode(canonicalize(sbom));
  const sourceLockCanonicalSha256 = await sha256Hex(canonicalize(source));
  if (serialized.includes(source.sbomSha256) || serialized.includes(sourceLockCanonicalSha256) || /sourceLockHash|timestamp|[A-Za-z]:\\|\/Users\//.test(serialized)) fail("SBOM contains a forbidden digest, timestamp, or host path");
  const rootComponent = { type: "application", "bom-ref": "pkg:pypi/hermes-agent@0.20.6", name: "hermes-agent", version: "0.20.6", purl: "pkg:pypi/hermes-agent@0.20.6" };
  record(sbom.metadata, ["component", "properties"], "SBOM metadata"); compareCanonical(sbom.metadata.component, rootComponent, "SBOM metadata component"); compareCanonical(array(sbom.metadata.properties, "SBOM metadata properties"), runtimeProperties(source), "SBOM metadata properties");
  ...
  if (distributions.length !== 65 || provenance.length !== 14 || components.length !== 79 || dependencies.length !== 66) fail("SBOM does not contain the complete selected Windows closure and provenance");
  const declaredRefs = new Map([[rootComponent["bom-ref"], rootComponent]]);
  for (const component of components) {
    const reference = component?.["bom-ref"];
    if (typeof reference !== "string" || reference.length === 0 || reference.trim() !== reference || /\s/.test(reference) || declaredRefs.has(reference)) fail("SBOM component bom-ref is absent, invalid, or duplicated");
    declaredRefs.set(reference, component);
  }
  const byRef = new Map([[rootComponent["bom-ref"], rootComponent]]);
  const distributionRecords = [{ name: rootComponent.name, version: rootComponent.version, type: rootComponent.type, purl: rootComponent.purl }]; const archiveRecords = [];
  for (const component of distributions) {
    record(component, ["bom-ref", "externalReferences", "hashes", "name", "properties", "purl", "type", "version"], "SBOM distribution component");
    ...
    literal(component.purl, purl(component.name, component.version), `SBOM ${component.name} purl`); literal(component["bom-ref"], component.purl, `SBOM ${component.name} bom-ref`); literal(component.type, "library", `SBOM ${component.name} type`); byRef.set(component["bom-ref"], component);
  ...
  distributionRecords.sort((left, right) => compareOrdinal(left.name, right.name) || compareOrdinal(left.version, right.version)); archiveRecords.sort((left, right) => compareOrdinal(left.name, right.name) || compareOrdinal(left.version, right.version));
  const closure = distributionRecords.map((component) => `${component.name}==${component.version}`).sort(compareOrdinal);
  ...
  for (const dependency of dependencies) {
    ...
    const dependsOn = array(dependency.dependsOn, "SBOM dependsOn"); if (dependsOn.some((reference) => typeof reference !== "string" || !byRef.has(reference)) || new Set(dependsOn).size !== dependsOn.length || dependsOn.some((reference, index) => index > 0 && compareOrdinal(dependsOn[index - 1], reference) >= 0)) fail("SBOM dependency edge drift");
    const component = byRef.get(dependency.ref); dependencyRecords.push({ name: component.name, version: component.version, dependsOn: dependsOn.map((reference) => byRef.get(reference).name).sort(compareOrdinal) });
  }
  if (seenDependencies.size !== byRef.size) fail("SBOM dependency coverage drift"); dependencyRecords.sort((left, right) => compareOrdinal(left.name, right.name) || compareOrdinal(left.version, right.version)); literal(await sha256Hex(canonicalize(dependencyRecords)), DEPENDENCY_RECORDS_SHA256, "SBOM dependency record hash");
  ...
  for (const component of provenance) {
    record(component, ["bom-ref", "name", "properties", "type", "version"], "SBOM provenance component"); if (typeof component.name !== "string" || !expectedByName.has(component.name)) fail("SBOM provenance component set drift"); compareCanonical(component, expectedByName.get(component.name), `SBOM provenance ${component.name}`); expectedByName.delete(component.name);
  }
...
async function runCli() {
  const [sourceFile, artifactsFile, contractFile, patchesFile, sbomFile, wireSchemaFile, wireGoldenFile, sourceSchemaFile, artifactsSchemaFile, packageFile] = await Promise.all([
    ...
    loadCanonicalJsonFile(new URL("../package.json", import.meta.url), "Hermes runtime package"),
  ]);
  ...
  checkRuntimePackage(packageFile.value);
