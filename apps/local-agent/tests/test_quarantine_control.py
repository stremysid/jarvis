    ...
    def client() -> None:
        try:
            ...
            if quarantined:
                ...
                assert f"projection_retry {fact_id} not_quarantined" in runtime.state.report()
...
def test_real_socket_retry_and_status_stay_responsive_during_a_slow_cloud_call(
    ...
    def client() -> None:
        try:
            ...
            assert f"projection_retry {fact_id} queued" in status.lines
            ...
            while True:
                status = send_unix_control_request(CliCommand("status"), settings.control_socket_path)
                if f"projection_retry {fact_id} {expected}" in status.lines:
                    break
...
def test_a_busy_cycle_returns_queued_and_reports_the_later_result() -> None:
    state = ServiceState()
    ...
    try:
        ...
        assert f"projection_retry {FACT_ID} queued" in state.report()
        coordinator.drain(lambda _fact_id: True)
        assert f"projection_retry {FACT_ID} applied" in state.report()
...
def test_a_retry_after_close_is_refused_without_queuing_or_waiting() -> None:
    state = ServiceState()
    coordinator = _QuarantineRetryCoordinator(state)
    coordinator.close()
    service = LocalAgentService(control_handlers(state, retry_quarantined=coordinator.submit))
...
def test_stopping_cancels_a_queued_retry_without_deleting_the_quarantine() -> None:
    state = ServiceState()
    coordinator = _QuarantineRetryCoordinator(state)
    service = LocalAgentService(control_handlers(state, retry_quarantined=coordinator.submit))
    client = threading.Thread(
    ...
    try:
        ...
        coordinator.drain(lambda fact_id: deleted.append(fact_id) is None)
        assert deleted == []
        assert f"projection_retry {FACT_ID} cancelled" in state.report()
        assert state.take_cycle_request() is False
    finally:
        coordinator.close()
        client.join(timeout=1)
...
def test_a_process_interrupt_cannot_report_fact_not_quarantined() -> None:
    state = ServiceState()
    coordinator = _QuarantineRetryCoordinator(state)
    service = LocalAgentService(control_handlers(state, retry_quarantined=coordinator.submit))
    responses: list[CliResponse] = []
    ...
    try:
        ...
        client.join(timeout=1)
        assert responses[0].code == "retry_failed"
        assert f"projection_retry {FACT_ID} failed" in state.report()
    finally:
        coordinator.close()
        client.join(timeout=1)


def test_status_keeps_all_pending_retries_and_only_the_latest_completed_results() -> None:
    ...
    try:
        ...
        assert f"projection_retry {ids[0]} applied" not in lines
        assert all(f"projection_retry {fact_id} applied" in lines for fact_id in ids[1:3])
        assert all(f"projection_retry {fact_id} queued" in lines for fact_id in ids[3:])
        assert len([line for line in lines if line.startswith("projection_retry ")]) == 4
    finally:
        coordinator.close()
...
def test_a_mid_cycle_retry_is_resolved_after_commit_or_cancelled_on_stop(tmp_path: Path, stop: bool) -> None:
    ...
    try:
        ...
        assert f"projection_retry {fact_id} {outcome}" in runtime.state.report()
...
def test_runtime_cancels_inflight_retry_before_joining_control(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert f"projection_retry {FACT_ID} cancelled" in runtime.state.report()


def test_refused_control_work_preserves_the_original_backoff_deadline() -> None:
    state = ServiceState()
    coordinator = _QuarantineRetryCoordinator(state)
    moment = [0.0]
    delays: list[float] = []
