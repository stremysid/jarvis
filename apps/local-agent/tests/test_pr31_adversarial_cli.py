...
def test_real_store_missing_key_is_refused_and_creates_no_file(
    ...
    assert captured.out.strip() == MISMATCH
    assert captured.err == ""
...
def test_real_sealed_key_begin_sends_the_number_only_inside_the_signed_body(
    ...
    assert json.loads(begin_request.data) == {"operation": "begin", "phoneNumber": PHONE, "schemaVersion": "1.0"}
    ...
    assert prompts["getpass"] == ["Phone number in E.164 form (input hidden): "]
