...
export class VoiceAccessRepository {
  ...
  #authorityMatchesBinding(row: AuthorityRow, relayBinding: RelayBinding, nowIso: string): boolean {
    ...
    if (row.authority_kind === "owner") {
      // The passphrase waiver or success receipt this used to require is gone
      // with the per-call gate (Sid, 2026-09-24), and nothing can write either
      // row any more. The durable enrolment check is what remains: the authority
      // is only current while the session's owner identity is the enrolled one.
  ...
  async requireCurrentAuthority(input: PersistedCallAuthority, now: Date = new Date()): Promise<PersistedCallAuthority> {
    ...
    if (authority.kind === "owner") {
      // The per-call passphrase success or waiver this used to require is gone
      // with the gate, and nothing can write either row any more: keeping the
      // check would make every owner turn stale. The durable enrolment check
