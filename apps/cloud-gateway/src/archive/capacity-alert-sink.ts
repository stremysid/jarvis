...
import type { CapacityAlert, CapacityAlertSink, CapacityResource } from "./capacity-guard.js";

interface SinkOptions {
  database: D1Database;
  ownerPrincipalId: string;
  telegram: TelegramProvider;
  migrationResources: readonly CapacityResource[];
  now: () => Date;
}

function unavailable(): Error { return new Error("capacity_alert_unavailable"); }
function key(value: string): string {
  if (!/^capacity:(?:d1|r2|provider:[a-z0-9][a-z0-9_-]{0,63}):(?:70|85)$/u.test(value)) throw unavailable();
  return value;
}
...
export class D1CapacityAlertSink implements CapacityAlertSink {
  ...
  private readonly migrations: ReadonlySet<CapacityResource>;
  ...
  constructor(options: SinkOptions) {
    ...
    this.migrations = new Set(options.migrationResources);
    this.now = options.now;
  }

  async emit(alert: CapacityAlert): Promise<void> {
    const alertKey = key(alert.idempotencyKey);
    if (alertKey !== `capacity:${alert.resource}:${alert.threshold}` || alert.code !== `capacity_${alert.threshold}`) throw unavailable();
    const now = this.now();
    ...
    const text = this.migrations.has(alert.resource)
      ? `Jarvis ${alert.resource}: ${alert.threshold}% of the configured prepaid allocation is used. Plan the switch to the next provider (R7). This is a migration reminder based on reported credit.`
  ...
  async rearm(idempotencyKey: string): Promise<void> {
    await this.database.prepare("DELETE FROM capacity_alert_crossings WHERE owner_principal_id = ? AND alert_key = ?")
      .bind(this.owner, key(idempotencyKey)).run();
  }
