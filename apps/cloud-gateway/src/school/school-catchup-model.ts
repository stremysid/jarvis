...
- Use an existing itemId or unique new-item-N reference. New items require an existing programId or a new-N programRef created in the same response, kind, label, status, statusEvidence copied from owner_message_json, and a dueDate object. Use an unverified null date when the owner supplied no current-cycle date. On existing items, kind and label are null, and null status or dueDate means no change.
- You choose the status: not_started, drafting, ready, submitted_by_sid or not_needed_by_sid, from what Sid means, including a correction of an earlier status and a reactivation after not_needed_by_sid. Nothing in code reads his wording for the status. Every non-null statusEvidence is the whole current owner message; it is the provenance for the status you declare. Store a due date only when Sid supplies one: dueDate.date is an ISO YYYY-MM-DD date and dueDate.evidence is the whole current owner message. Never invent or infer a date, and never guess an application item, program or requirement.
- inactiveApplicationItems contains only submitted or not-needed history named by the current owner message. Use its itemId only for a correction or reactivation you judge from that message.
...
- Use an existing id or new-workflow-N. New actions link one application item; offer kinds use null. New rows require identity, status, whole-message evidence, deadline and owner_only. Existing rows keep identity null and change status, draft/checklist text or deadline. You choose prepared or the owner-reported status that matches what Sid said, and you declare the provenance: statusEvidence is the whole current owner message. Judge from that message; never infer an action from a page or another person. Store no invented fact, date or fee. deadline carries date or exact UTC plus IANA timezone, verification, and whole-message evidence.
