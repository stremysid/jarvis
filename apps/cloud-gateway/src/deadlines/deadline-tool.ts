...
export async function recordDeadline(database: D1Database, input: Readonly<ModelAdapterStreamInput>,
  ...
  try {
    ...
    const evidence = normalize(excerpt.replace(VERTICAL_BREAK, ", "));
