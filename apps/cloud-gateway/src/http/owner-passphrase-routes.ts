...
import { OwnerPassphraseService, OWNER_PASSPHRASE_PATH } from "../sync/owner-passphrase.js";
...
const AUTH_FAILURES = new Set([
  ...
  "signed_request_expired", "replayed_nonce", "device_key_changed", "owner_passphrase_device_mismatch",
]);
  ...
  try {
    ...
    return {
      ...
      pepper: decodeCanonicalBase64(
        env.OWNER_PASSPHRASE_PEPPER_V1, 32, "owner_passphrase_configuration_invalid",
      ),
    };
  } catch {
    return null;
...
export function isOwnerPassphrasePath(pathname: string): boolean {
  return pathname === OWNER_PASSPHRASE_PATH;
}

/** Production route for signed status and Worker-side generation. */
export async function handleOwnerPassphraseRequest(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    return response(400, { error: "owner_passphrase_request_rejected" });
  }
  ...
  if (configuration === null) {
    try {
      await new DeviceRequestVerifier({ database: env.DB, audience: SYNC_AUDIENCE }).verify(
        envelope, "POST", OWNER_PASSPHRASE_PATH, body, rawBody, new Date(), (value) => value,
      );
    } catch (error) {
      return failureResponse(error);
    }
    return response(503, { error: "owner_passphrase_not_configured" });
  }
  try {
    const service = new OwnerPassphraseService({
      database: env.DB,
