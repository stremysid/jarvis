...
describe("CallSession production composition", () => {
  ...
  it("gives an owner's call memory and shared argument tools in the configured owner zone", async () => {
    ...
    expect((body.tools as { function: { name: string } }[]).map((tool) => tool.function.name))
      .toEqual([...MEMORY_TOOL_DEFINITIONS, ...OWNER_ARGUMENT_TOOL_DEFINITIONS].map((tool) => tool.name));
