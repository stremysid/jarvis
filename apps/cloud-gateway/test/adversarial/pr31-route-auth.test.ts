...
import {
  ...
  OTHER_PHONE, OWNER_IDENTITY, PEPPER, PHONE, PREFLIGHT, resetEnrollmentState, seedDevice, seedPrincipal, send,
  signParts, stateCounts, STATUS, type Device,
...
const CANONICAL_BEGIN_TEXT = '{"operation":"begin","phoneNumber":"+15005550006","schemaVersion":"1.0"}';
...
describe("PR31 adversarial 1: authentication bypass on the enrollment route", () => {
  ...
  it("1k refuses validly signed non-canonical bodies (whitespace, duplicate key, BOM) without state", async () => {
    const variants = [
      '{"operation":"begin", "phoneNumber":"+15005550006","schemaVersion":"1.0"}',
      '{"operation":"preflight","operation":"begin","phoneNumber":"+15005550006","schemaVersion":"1.0"}',
...
describe("PR31 adversarial 2: the body cannot supply trusted identity", () => {
  ...
  it("2c refuses an own __proto__ member carrying a principal", async () => {
    const raw = encoder.encode(
      '{"__proto__":{"principalId":"principal:attacker"},"operation":"begin","phoneNumber":"+15005550006","schemaVersion":"1.0"}',
    );
