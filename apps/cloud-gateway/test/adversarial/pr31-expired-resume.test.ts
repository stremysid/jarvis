...
import {
  AUDIENCE, BEGIN, enrollmentEnvironment, KEY_VERSION, OWNER_IDENTITY, PEPPER, resetEnrollmentState, seedDevice,
  seedPrincipal, send, STATUS, type Device,
...
// v2 (327ddda). INVERTED tests pass on the fix and fail if the finding returns.
async function confirmShown(device: Device, shown: { challengeId: string; response: string }) {
...
describe("PR31 v2 inverted 5a / 5a': resuming an expired enrollment", () => {
  let home: Device;
  ...
  it("INVERTED 5a the same-phone retry after expiry returns 200 with exactly the challenge it committed", async () => {
    const first = await send(home, BEGIN, enrollmentEnvironment());
    expect(first.status).toBe(200);
    const shown = first.json as { challengeId: string };
    ...
    const retried = retry.json as { enrollmentState: string; challengeId: string; response: string; expiresAt: string };
    expect(retried).toMatchObject({ enrollmentState: "pending", expiresAt: iso(601_000) });
    expect(retried.challengeId).not.toBe(shown.challengeId);

    const rows = await challengeRows();
    expect(rows.map((row) => row.challenge_id)).toEqual([retried.challengeId]);
    expect(rows[0]?.consumed_at).toBeNull();
    expect((await send(home, STATUS, enrollmentEnvironment())).json).toMatchObject({ enrollmentState: "pending" });
    // The response the owner was shown is the live one.
    await expect(confirmShown(home, retried)).resolves.toMatchObject({ state: "active" });
  });

  it("INVERTED 5a' a first-ever begin beside an unrelated expired challenge returns 200 with the committed challenge", async () => {
    await env.DB.batch([
      env.DB.prepare(
        `INSERT INTO channel_identities
           (identity_id, principal_id, channel, provider_subject, status, verified_at, created_at, enrolled_by_device_id)
    ...
    const issued = response.json as { enrollmentState: string; challengeId: string; response: string };
    expect(issued.enrollmentState).toBe("pending");
    expect(await challengeRows()).toMatchObject([{ challenge_id: issued.challengeId, channel: "voice", consumed_at: null }]);
    expect(await env.DB.prepare("SELECT principal_id, identity_id FROM voice_owner_identity").first())
      .toEqual({ principal_id: "principal:owner", identity_id: OWNER_IDENTITY });
    await expect(confirmShown(home, issued)).resolves.toMatchObject({ state: "active" });
  });
});
