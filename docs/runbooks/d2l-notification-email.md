...
2. From a real D2L notification already visible in the school mailbox, record
   only the exact domain after `@` in its visible `From:` address. Do the same
   for a real Google Classroom notification if Classroom mail is enabled. Do
   not guess either domain and do not substitute the Microsoft forwarding
   tenant or an envelope-sender domain.
...
Store the three values only through Wrangler's interactive secret prompt. Use
...
& node $wrangler secret put GOOGLE_CLASSROOM_EMAIL_FROM_DOMAINS --config $gateway --env ''
& node $wrangler secret list --config $gateway --env ''
```

The domain values are comma-separated exact domains, without `@`, spaces,
wildcards, or URLs. Keep `untrusted.invalid` for a source that is not yet being
accepted; never broaden a pin to `google.com` or a board parent domain merely
to make setup pass. The list must show all three variable names; record names
and presence only, never values. These commands update configuration but do
not by themselves prove the route or parser works.
...
4. D2L sends an address-verification message. Wait for Jarvis's fixed Telegram
   ...
   opens the link or enters the code himself; Jarvis never follows it.
5. Return to **Notifications**. Under **Instant Notifications**, select email
...
- the stored header-name list shows what Cloudflare and Microsoft actually
  supplied. Authentication-header absence remains `unknown`, not `pass`.
