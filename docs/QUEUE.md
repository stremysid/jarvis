...
`#145` and `#151` are open, and they are independent of each other. Everything else merged on
2026-09-21/22 — see the merged list below the table.

| PR | State | Next action | Owner | BLOCKS | Notes |
|---|---|---|---|---|---|
| [#145](https://github.com/stremysid/jarvis/pull/145) | **ready-to-merge** | Merge whenever; independent of everything else | Sid | **Phase 3 and Phase 4** | `jarvis serve` binds the Windows control pipe through the existing `NamedPipeServer`, so the boot chain reaches a live agent instead of exiting 3. **This unblocks P2, the PC reading D2L** — the only remaining route to Phase 3's exit test. To work at logon it also needs `JARVIS_ARCHIVE_PATH` and `JARVIS_MEMORY_PATH` at user scope, and the boot script exits 0 the moment the pipe answers, so a device the gateway rejects leaves a service that vanished seconds later |
...
| [#96](https://github.com/stremysid/jarvis/pull/96) | **blocked** | **Rebase onto [#149](https://github.com/stremysid/jarvis/pull/149)**, which merged and changed the same rule. Its migration is numbered `0036`, below the applied `0038`, so an ordered apply would run it after. Taken numbers are `0001`–`0035` and `0038`; the next free is `0039`, and it must take whatever is next when it lands | builder | Phase 5 | Spoken PIN before sensitive actions, and the redaction fix. 57 files |
...
| [#117](https://github.com/stremysid/jarvis/pull/117) | **superseded in part** | Keep only the CI wiring for `check-state.mjs`; its carrier content is superseded. **The CI wiring is the durable half** — nothing runs `check-state.mjs` today | reviewer | none | Reviewer-authored |
...
**Merged on 2026-09-21/22:** `#133`, `#137`, `#138`, `#139`, `#140`, `#141` (`e3e3527`), `#143`,
...
| **PC controls: boot chain, D2L read, daily report** | **P1 merged; the launcher is open in [#145](https://github.com/stremysid/jarvis/pull/145); P2 has a brief; P3 not started** | Three stacked PRs. **P1** auto-login plus a logon-scheduled task at `-RunLevel Highest` — merged as `e3e3527`. **P2** is [briefs-p2-d2l-read.md](briefs-p2-d2l-read.md): the PC logs into D2L and reads assignments, pushing through the already-enrolled device, and **most of the cloud half already exists** — `school-observation-repository.ts`, `classroom-observation-sync.ts` (whose page-reader is an interface, not Classroom) and the `school_assignment_observations` schema, so the gap is the reader, not the store. **P3** is the report on Telegram, and it must state `last_success_at` — a scraper that breaks silently is the failure that matters. Overview: [briefs-pc-controls.md](briefs-pc-controls.md). **This is the only route to Phase 3**, because the D2L email carries no deadlines | builder | **Phase 3 and Phase 4** |
| **No Windows launcher for the local agent** | **built and open in [#145](https://github.com/stremysid/jarvis/pull/145)** | `jarvis serve` binds `\\.\pipe\jarvis-local-agent` through the existing `NamedPipeServer`, so `ops/jarvis-boot.ps1` resolves a real command and the chain reaches a live agent. `jarvis config` answers what `serve` would refuse on without starting it. **Two things remain before it works at logon:** `JARVIS_ARCHIVE_PATH` and `JARVIS_MEMORY_PATH` at user scope (see [OWNER-ACTIONS.md](OWNER-ACTIONS.md)), and the boot script exits 0 the moment the pipe answers, so a rejected device leaves a service that vanished seconds later — the logon task restart count is the only recovery | builder | none |
