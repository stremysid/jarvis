...
| Voice has no tool dispatch — a call can talk and cannot act | **decided, not started** | Compose the tool-calling agent into `CallSessionCore`, with the tier-3 gate in front and receipts on the voice channel. **Scope is larger than "compose the agent", measured 2026-09-21:** voice composes `D1ContextRetriever` (`voice/production-runtime.ts:110`), which implements **`ContextRetriever` only**, while `TelegramMemoryRetriever` also implements `TelegramMemoryTargetFinder`. **So a voice turn cannot resolve an item id at all.** The adapter must bring the finder with it, or any tool taking an `itemId` (forget, correct, restore, confirm, explain, pin) will be unusable on voice from the first call. **None of that is currently asserted by a test** — both retriever suites stub `findControlTargets`, so which tools need it is a read, not a measurement | builder | **Phase 5** |
