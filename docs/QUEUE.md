...
| **No Windows launcher for the local agent** | **built; merged in [#145](https://github.com/stremysid/jarvis/pull/145) (`ec5ebb5`)** | `jarvis serve` binds `\\.\pipe\jarvis-local-agent` through the existing `NamedPipeServer`, so `ops/jarvis-boot.ps1` resolves a real command and the chain reaches a live agent. `jarvis config` answers what `serve` would refuse on without starting it. **Before it works at logon:** `JARVIS_ARCHIVE_PATH` and `JARVIS_MEMORY_PATH` at user scope (see [OWNER-ACTIONS.md](OWNER-ACTIONS.md)). The review's lingering-service finding was fixed before merge: at `ec5ebb5`, `NamedPipeServer.close()` wakes the parked control thread and the process exits (re-checked on the home PC 2026-09-23). Nobody has yet observed the agent outliving the boot script under Task Scheduler | builder | none |
