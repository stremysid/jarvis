...
| [#179](https://github.com/stremysid/jarvis/pull/179) state carriers | **round 4 in progress**; round 3 cleared the main merge at `70f5a29` | After this docs round, obtain review of the resulting head | harness, then reviewer | current carrier evidence | This branch, `claude/friendly-hawking-qjcyia` |
...
| [#184](https://github.com/stremysid/jarvis/pull/184) call-session fixes | **round 1 requested changes**; `a035630` | In the builder round, address the round-1 findings and request review | builder, then reviewer | call-session fixes | The review started on `e786602`; `a035630` is a harness merge of main, and the PR's own diff against main is byte-identical at both heads |
| [#185](https://github.com/stremysid/jarvis/pull/185) CI flake bounds | **round 1 requested a small fix**; `8822708` | In the builder round, raise the two Hermes per-test timeouts that are still shorter than the child deadline they wrap, then request review | builder, then reviewer | CI reliability | The local-agent half is correct as-is. Covers `test_node.py`'s race against `QUARANTINE_RETRY_WAIT_SECONDS = 0.1` and `canonical-closure-review3.test.mjs:51` hitting Vitest's default 5 s limit on Windows |
...
#181 merged as `68675ba` at 21:31 UTC on 2026-09-24 and is tonight's release revision.
...
| Deadline "proof contract" refusals | **Sid decided to remove; not started** | Remove `DUE_PHRASE`, the "code cannot choose" refusals and the `statusOf` keyword list in `apps/cloud-gateway/src/deadlines/deadline-date-proof.ts`; the model decides, asks when unsure, and code checks only that the date exists and keeps the raw message as the receipt. This replaces the former #166-round-7-Z question; Sid's 2026-09-24 statement is in [OWNER-ACTIONS](OWNER-ACTIONS.md) | builder | the deadline tool's refusals |
