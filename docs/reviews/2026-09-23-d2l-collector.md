...
- #161 observed the entity `submissions/` response, not `mysubmissions/`, and did not observe myItems or populated grades/submissions. The later owner-run probe observed `mysubmissions/` and grades as arrays, so current fixtures preserve those newer shapes with invented values; positive own-status examples remain explicitly synthetic. Unknown shapes remain raw evidence with projection labels.
