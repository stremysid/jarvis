...
|---|---|---|
| R0 Green and deployed | **UNMET** (declared passed 2026-09-11) | CI runs again since 2026-09-19 and its last red job, `local-agent`, was fixed by #115. The watchdog has still never recorded a gateway heartbeat, and that clause was removed from the exit test rather than met |
...
| Gate | State |
|---|---|
| CI | **Alive again since 2026-09-19**, after the repository moved into the organisation — the 2026-10-01 reset is no longer what gates it. `local-agent` was the only red job and #115 fixed it. Query it: `gh run list --repo stremysid/jarvis --branch main` |
...
`hermes-runtime` is green on CI, so `$KnownPreExistingFailures` in
`reviewer-tools/gate.ps1` — four named hermes tests — is **stale**. CI is the
authority on which tests fail, not a list in a script.

**Consequence, stated plainly: a green local run may not be banked, and a red one
cannot be attributed.** `testTimeout` is still unset, which is what makes the
workspace suite unattributable; that is the next thing to fix, not more measuring. Until that changes, nothing may be merged on the strength of
a local run alone.
