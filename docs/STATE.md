...
| 3 School | **built, and cannot receive anything useful** | The email route is configured, but D2L notifications carry neither assignment nor date; Classroom credentials are unavailable and there is no calendar feed ([FACTS.md](FACTS.md)). A logged-in reader is needed. P2 proposes the PC; [2026-09-23 design research](plan/2026-09-23-d2l-access-design.md) recommends a Windows browser extension with P2 as fallback, **conditional on unrun owner experiments**. The existing observation interface is injectable but its types, repository and schema still enforce Classroom provenance. Neither route is verified against the school. The prior claim that only the PC could read D2L was unsupported. See [QUEUE.md](QUEUE.md) |
