...
| 3 School | **built, and cannot receive anything useful** | **Corrected 2026-09-21:** this previously read *"receiving nothing"* and blamed the Cloudflare routing rule. The routing has since been configured correctly and the real blocker is upstream — **D2L's email carries no deadline.** Sid enabled every notification option; D2L sends an activity summary naming the course with a count (*"76 New Emails"*) and a link. No assignment, no date. Per course and one for the board, both linking to a login-walled inbox. So the handler, parser and authenticity checks are correct and **their input cannot contain what they need**, and the dates are behind authentication. Classroom is impossible on this board (no Google Cloud Console access) and the Brightspace feed does not exist. **The only remaining route is the PC reading D2L while logged in** — Phase 4/5 work, not a cloud job |
