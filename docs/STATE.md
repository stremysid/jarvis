...
Sid, 2026-09-23: **"THE ONLY difference between call and telegram is the method of communication, THAT'S IT."** In code (#174, not deployed), both channels share one `OWNER_TOOL_DEFINITIONS` catalogue (18 tools from #174; 19 with #198's `history_search`), `OwnerAgentCore`, one prompt core, one claim policy, the same canonical recall and the same Telegram tap for inferred memory and tier 3; guests get no owner profile, framing or tools.
