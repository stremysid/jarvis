...
| 4 Control | **built as the inverse of what the roadmap asks** | Tiers are a D1 table looked up per capability, not prompt guidance Jarvis judges. `codex/tool-gate-binding` binds confirmations to tool name, capability and argument hash across channels and denies a changed second outcome; harness validation and independent review are pending. Old references require a fresh tap after deployment; see [the compatibility note](../KNOWN_ISSUES.md#tier-3-confirmations-issued-before-tool-binding-2026-09-24). The [single-use tap change](reviews/2026-09-23-tier3-tap.md) claims each tap once with a ten-minute expiry; migration `0039` and the gateway rollout await owner action |
