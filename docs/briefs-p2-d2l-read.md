...
- **A `D2LIfLoggedIn` or `isSchoolHours` guard.** The first decides whether to act; the second
  makes code decide when Jarvis acts. Same shape as `callPlaceIfSensitive`, recorded as the
  thing to refuse in `docs/briefs-pc-controls.md` (it is not a row in `docs/CODE-VS-JUDGMENT.md`).
