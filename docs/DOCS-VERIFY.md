...
| 10 | A late split passphrase repeat is ordinary conversation (PR #40 N9) | **Removed 2026-09-24:** `#guardOwnerRepeat` and the whole repeat path were deleted with the per-call passphrase gate (PR #196), so there is no late fragment to suppress. |
| 11 | Owner-call step-up has five deferred failure and concurrency edges | **Removed 2026-09-24** with the step-up service (PR #196). The admission-SQL note survives under item 11's original subject only in [KNOWN_ISSUES](KNOWN_ISSUES.md)'s outbound-slot row. |
| 12 | Owner-call passphrase boundary awaits rollout and live acceptance | **Removed 2026-09-24:** there is no owner-call passphrase boundary any more (PR #196). The surviving requirement is a live smoke against schema `1.4`; missing outbound/live-release acceptance was retained and is in [KNOWN_ISSUES](KNOWN_ISSUES.md). |
| 13 | R1 live voice evidence has two deferred observability limits | Retained per-effect observability and failed-attempt-ledger gaps. Five-scenario contract verified in source; no paid call performed. |
