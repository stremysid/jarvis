...
Run:
    ...
    if ($smokeExit -eq 0 -or $smokeOutput -notmatch 'live_execution_not_authorized') { throw 'unexpected_voice_smoke_outcome' }
...
Expected: full suite 68 files/1,240 tests, typecheck, lint, and audit pass; the default smoke reports `live_execution_not_authorized`; the release gate exits nonzero only with `release_voice_evidence_incomplete` because H0 does not fabricate live-call evidence.
...
Run:
    ...
    pnpm test:voice-smoke
    $smokeOutput = (& pnpm smoke:voice -- --scenario inbound 2>&1) -join "`n"
    if ($LASTEXITCODE -eq 0 -or $smokeOutput -notmatch 'live_execution_not_authorized') { throw 'unexpected_voice_smoke_outcome' }
    $gateOutput = (& pnpm release:voice-gate 2>&1) -join "`n"
