...
5. A per-call Durable Object validates the WebSocket handshake, accepts structured speech and DTMF events, and begins in `pre_auth` state. It may emit only a neutral greeting and authentication prompt in this state.
6. Sid enters an eight-digit DTMF PIN. The authentication service verifies a versioned PBKDF2-HMAC-SHA-256 record containing a cryptographically random 128-bit salt, at least 600,000 iterations, and digest; the service rehashes after successful authentication when policy increases the work factor. The initial verifier is installed only by the one-time bootstrap transaction; rotation requires an authenticated local command confirmed on an already enrolled channel. The verifier is stored as a Worker secret. PIN digits bypass the transcript, model, event payloads, and logs. Three failures terminate the call, but an unauthenticated Caller-ID claim never locks the canonical identity. No personal memory or purpose is loaded before success.
...
Caller ID selects only a candidate identity and never grants memory access. Every inbound call requires both an allowlisted number and the DTMF PIN. Authentication failures produce no model request and no personal disclosure. Failure throttles apply to the current `CallSid`, a short rolling composite source bucket, and a global abuse budget; they expire within five minutes and never disable Sid's canonical identity. Sid can clear throttles through authenticated Telegram or local CLI recovery. Tests must prove a spoofed caller cannot create a persistent lockout.
...
6. When answered, Jarvis gives only a neutral identity statement and requests the same DTMF PIN. It does not disclose the call purpose or memory-derived content to voicemail, another person, or an unverified respondent. After three failures it ends the call. Voicemail receives only: "Jarvis called for Sid. No private message was left."
7. After recipient verification, Jarvis states the authorized purpose and continues. Busy, no-answer, rejected, and failed outcomes become events. A retry retains the original authorization, correlation, and idempotency lineage and is blocked after expiry.
