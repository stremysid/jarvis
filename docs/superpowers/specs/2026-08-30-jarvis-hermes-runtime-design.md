...
- status reconciliation parses the exact hash-locked `GET /v1/runs/{run_id}` union for `queued`, `running`, `stopping`, `completed`, `failed`, and `cancelled`; the first three are nonterminal and only the last three settle a run. The zero-tool `running` state accepts either no `last_event` or exactly `last_event:"reasoning.available"`; every tool/approval/subagent/steer callback marker is capability drift. Polling is bounded by the original persisted deadline. A terminal `failed` or `cancelled` maps honestly; a 404 after restart/TTL, transport loss, malformed status, or deadline exhaustion leaves the already-admitted outcome ambiguous, persists `model_cancel_unknown`, quarantines the session, and permits neither replay nor fallback.
...
The H1 gateway exists only in an inert local Wrangler configuration with three closed modes: `fake`, `pinned_runtime`, and `live`. Each mode is selected only by its named local Wrangler environment and is bound to an exact expected configuration hash. Fake is the only mode permitted to construct the synthetic direct adapter. Pinned-runtime and live modes construct only a fail-closed sentinel direct adapter and reject cross-mode attestation before provider traffic, so a real-runtime 503 cannot masquerade as model success. No request, header, URL, credential file, or deployed Worker binding can select an H1 mode.
...
The bounded H1 ledger serves existing exact replays even at its retention ceiling. Its 1 GiB ceiling is the logical sum of persisted request/response/frame payload bytes plus unconsumed per-run payload reservations, not the platform-dependent SQLite file size. For a valid, authenticated request with no existing record, the bridge computes a deterministic worst-case durable footprint from the exact request bytes, fixed metadata, contract frame caps, maximum token-frame count, worst-case JSON escaping, cancellation bodies, and terminal frame. Under the same write transaction it compares current logical payload plus all outstanding reservations plus that request's worst case against 1 GiB, reserves the accepted amount before admission, or returns the closed non-fallback `ledger_capacity_exhausted` response without creating a record or contacting Hermes. Appends consume but cannot exceed the reservation; terminal settlement releases only the unused remainder, and crash recovery preserves every outstanding reservation. The record-count decision is atomic in the same transaction. A concurrent boundary race may reserve at most the capacity that remains; every loser gets the same overload outcome. A structurally healthy ledger remains readiness-routable at either retention ceiling so exact replay and the per-request 507 decision stay reachable; capacity headroom is not a readiness predicate. Integrity, quarantine, storage-I/O, or attestation failure still fails readiness. Offline rotation remains the only way to restore new-request admission.

The separate cancellation endpoint accepts only a matching request whose validated 200 stream already proves the bridge run is durably bound. An absent or never-bound cancel returns opaque 404 without a write/upstream effect, and changed material returns 409. The gateway never calls cancel before bound proof: abort/loss before exact response headers remains an admission outcome. Exact 503 needs no cleanup or new Hermes admission and permits only the nominal direct fallback with the same frozen input and correlation ID; exact 507 needs no cleanup and remains nonfallback. Any ambiguous response settles `model_admission_unknown` with no retry or fallback. The admission handler itself tracks client disconnect; if observed before dispatch it commits a bounded `pre_response_aborted` tombstone when capacity permits or uses the same no-row 507 outcome at the ceiling. If dispatch already committed, the handler persists internal cancellation intent and binds/stops a later 202 before accepting output.
...
Jarvis abort immediately closes downstream delivery and discards every later upstream delta. After exact 200 stream headers prove the bridge run bound, adapter-generator `finally` calls an authenticated, idempotent Brain Bridge cancellation operation keyed by H1 `correlationId`/`requestId` or H2/H3 Brain `requestId`; before that proof it uses only the frozen admission settlement above. The adapter never learns a Hermes `run_id`, native endpoint, or native API key. The bridge looks up the bound ledger entry, durably records `cancel_requested`, and is the only component that posts native Runs `/stop`. A repeated cancel or lost bridge response resumes the same cancellation record and never issues a second logical stop.
...
- Plugins, the proposal guard/tool, model catalogs, provider routes, and any later read-only backend are separately hash-pinned and include-listed. H1 sets the remote Hermes model catalog disabled and installs immutable hash-locked deny snapshots for both catalog cache locations; `models.dev` is pointed at an invalid non-network scheme while exact DeepSeek model metadata comes only from the managed configuration. Mutable/stale catalog caches, ETags, background refresh, or any catalog network attempt fail readiness. An unexpected resolved tool, hook, plugin, model route, or configuration value fails readiness.
...
The provider advertises pre-compress checkpoint API v2, and every trusted profile keeps `compression.checkpoint_required: true` as the latent fail-closed guard. The single-turn H1 profile additionally sets `compression.enabled: false`; while it remains exactly disabled, readiness requires the checkpoint guard to stay armed but does not require an active checkpoint provider because no automatic lossy rewrite is reachable. Readiness fails if H1 compression becomes enabled without the selected v2-capable provider. Every later compression-enabled trusted profile requires that provider active and v2-capable. An archive/checkpoint failure preserves the uncompressed transcript, emits a controlled blocked outcome, and neither compresses nor continues best-effort. Tests prove H1 performs no automatic compaction and inject checkpoint failure before any later enabled compression to prove no transcript or authority loss.
