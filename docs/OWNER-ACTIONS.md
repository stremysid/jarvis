...
|---|---|---|
| Decide whether [#156](https://github.com/stremysid/jarvis/pull/156)'s local attestation gate may change temporary fixture modes | The fix-round instruction forbids permission changes, while `apps/hermes-runtime/test/attestation-contract.test.mjs` calls `chmod` on its disposable fixtures at lines 111, 224 and 239 | Asked 2026-09-23. Until Sid permits those calls or accepts the narrowed gate, those seven tests remain unrun in fix round 1. The other 15 permitted Hermes files passed 151 tests; source-lock and workflow-containment were excluded as requested. No permission change was attempted |
