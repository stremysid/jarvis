...
|---|---|---|
| Try one real D2L assignment paste after the school-paste change is reviewed, merged and deployed | Only Sid has the real assignment list and can confirm that the saved per-course notes and proposed daily workload match his pinned capacity. Paste as direct Telegram text; check the receipt includes work beyond today's tasks. No migration is needed for this change | **waiting for reviewed deployment** |
