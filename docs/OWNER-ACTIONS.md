...
| Roll out single-use tier-3 taps (#159 merged) | [0039 scratch rehearsal](reviews/2026-09-24-scratch-d1-rehearsal.md), 2026-09-24: The Claude orchestrator ran it; results are transcribed from its harness-supplied summary and table; not independently re-run. Sid, 2026-09-24 (orchestrator chat, supplied by the harness): '2: yes'. Apply `0039_tool_confirmation_consumptions.sql` before the matching gateway deploy; new code fails closed on tier 3 without the schema. Let pre-cutover taps age out for ten minutes after old code stops, then check fresh confirmations on a supported tool. [Rollout and evidence](reviews/2026-09-23-tier3-tap.md) | **scratch done / orchestrator-reported PASS; production application and deploy not started** |
