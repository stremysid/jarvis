...
| Set `JARVIS_ARCHIVE_PATH` and `JARVIS_MEMORY_PATH` **at user scope** | Needed now: #145 merged 2026-09-23. `ops/jarvis-boot.ps1` asks the agent's own `config` before starting anything, and those two names are required, so the logon chain exits 4 with a message naming them until they exist for the account the task runs as. They are write paths for this machine's stores, which is your environment and not a builder's to create | **not started** |
