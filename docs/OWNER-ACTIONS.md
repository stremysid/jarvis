...
|---|---|---|
| Prove and apply reminder migration 0041 before deploying the reminder PR stacked on #166 | After independent review, use the [remote D1 scratch-proof runbook](runbooks/migration-scratch-proof.md) on a newly confirmed disposable database, then apply the reviewed migration in the owner's release workflow before deploying its tools/drain. Keep assigned order 0039 tier-3 taps, 0040 D2L collector keys, 0041 owner reminders; #159 now uses 0039. The builder only proved local test D1 constraints and applied nothing to a real database | **not started; before reminder deployment** |
| Accept Telegram reminder delivery and cancellation after deployment | Ask Jarvis to schedule a study reminder, verify its stored local time/zone, UTC instant and words, observe one delivery after that instant outside quiet hours, and confirm a cancelled reminder never arrives. A failed/unconfirmed reminder may already have arrived; check before replacing it. A rejected reminder was not delivered; authentication/rate-limit refusals remain pending for retry. Use the reviewed 0041 schema including rejected, before deploying | **not started; after migration and deployment** |
| Verify an owner-reported deadline after the deadline tool PR is reviewed and deployed | Live acceptance needs your Telegram conversation: say a course/title is due Friday at 11:59pm or tomorrow at 3pm, then check the owner-zone receipt and next digest. A missing clock must be labelled date-only. Mention a submitted deadline again and confirm it stays submitted. #166 adds no migration; main's #159 has its own release prerequisite | **not started; after deployment** |
