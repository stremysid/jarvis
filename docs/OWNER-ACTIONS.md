...
|---|---|---|
| Choose the owner-local hour when the deadline tool's **small-hours window ends** | Only you can say when late-night `today` or `tomorrow` should name both adjacent calendar dates instead of their literal calendar dates. Until you give one hour, `OWNER_SMALL_HOURS_END_HOUR` is `null`: `today` means the current date and `tomorrow` the next date at every hour. The separate `tonight at 12am/1am` clarification does not depend on this choice | **not set; answer after #166 round 4, before enabling the window** |
| Verify an owner-reported deadline after the deadline tool PR is reviewed and deployed | Live acceptance needs your Telegram conversation and authenticated call: state a course/title due tomorrow at 3pm or a bare clock later today, then check the owner-zone receipt and next digest. Next weekday or a bare weekday naming today must ask which date. A missing clock must be labelled date-only; a submitted deadline must stay submitted when mentioned again. #166 adds no migration; main's #159 has its own release prerequisite | **not started; after deployment** |
