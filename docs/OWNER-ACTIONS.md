...
|---|---|---|
| Deploy the Telegram callback preservation fix after its PR is independently cleared and merged | A decision ULID containing six consecutive digits can make a real Confirm tap fail every time. `codex/flake-telegram` fixes new callback ingress; no migration. After deployment, re-tap a pending confirmation to create a fresh callback receipt. This builder performed no production operation | awaiting review and merge |
