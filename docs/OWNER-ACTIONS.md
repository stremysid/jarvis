...
|---|---|---|
| First live check of owner voice streaming | After independent review and your explicit approval to use credentials and provider credit, run a bounded check of the reviewed build: one ordinary owner-call answer and one harmless, clearly named test memory. Record time to first spoken sentence, whether DeepSeek actually emits indexed tool-call fragments, exactly one completed tool receipt, the follow-up request, and spoken text matching the final transcript. Check failure/refusal with a request that cannot act; no completion claim should be heard. Keep only redacted timing/protocol metadata, never credentials, raw private text or phone identifiers. Offline fixtures follow [DeepSeek's reference](https://api-docs.deepseek.com/api/create-chat-completion/); they do not verify live compatibility or phone latency. Deployment remains a separate owner action. See [the streaming evidence](voice-streaming.md) | **not started; live behavior unverified** |
