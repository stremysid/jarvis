...
| Authorize the D2L receiver migration and deployment after reviews | The builder has not touched a real database or production. Quiesce collector uploads for the migration/deploy handoff. Apply assigned 0039 before 0040, then the independently reviewed `0044_school_collector_hosts.sql` in the complete ordered main migration set. It preserves existing LDSB deadline identities while adding host provenance. Rehearse on a separately authorized scratch database before real application, deploy the matching gateway, resume collectors and use the two-board acceptance row | **not started** |
