...
|---|---|---|
| Restore or identify the incident report before any permission-sensitive local work resumes | The docs audit on 2026-09-23 could not read `C:\Users\Sid\Downloads\jarvis-profile-incident.md`: the file is absent, and a bounded filename search in Downloads found no substitute. Existing PC safety restrictions remain in force | **awaiting-owner** |
...
| Configure and prove an external watchdog monitor when monitoring setup is authorized | The watchdog's own health endpoint cannot report total cron failure without an outside poller. No builder may sign up, spend money or configure a service for this audit | **not started** |
| **Deploy an explicitly approved main revision** | As of STATE's observation of source `352991e` (2026-09-21), later main changes were not deployed. `f6bab5b` (#150) has since added the clean-tree/exact-origin-main guard to `scripts/deploy.ps1 -Publish`; the former claim that no guard exists is false. The deploy checkout was not inspected in this audit. Resolve its state and use the [deployment runbook](runbooks/deploy.md) only when you authorize a release | **not started** |
