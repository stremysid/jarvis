...
| `hermes-runtime` passes on CI as of 2026-09-19, so `$KnownPreExistingFailures` in `reviewer-tools/gate.ps1` — four named hermes security tests — is **stale**. CI is the authority on which tests fail, not a list in a script or a prompt | The completed `main` run of 2026-09-19: `hermes-runtime suite (windows)` green, `local-agent` red on one assertion | 2026-09-19 | yes |
