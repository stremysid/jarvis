...
| **Migration `0038` is applied; the deployed gateway code is `352991e` (#135).** D1 is at `0038`; Worker `78cb6e98` was uploaded at 21:27:49Z, 18 s before #133 merged, so #133's model default and everything after it is **not deployed**. Sid then made three secret changes (active version `64a184ce`). Watchdog `c940f9b7` | `wrangler deployments list`; `d1_migrations` | 2026-09-21 | yes |
...
| **A logged-in student session on LDSB Brightspace can read the Valence API directly**: `enrollments/myenrollments`, `dropbox/folders` (assignments and full rubrics), `content/toc`, and `grades/values/myGradeValues` all return 200 JSON. Authorisation is checked **per route** — `users/whoami` returns 403 to the same session. So the school data does not require HTML parsing, and a valid identity alone does not grant a route | Five URLs loaded in Sid's own logged-in browser, 2026-09-23; status codes and field names recorded in `docs/research/2026-09-23-ldsb-brightspace-api-findings.md` | 2026-09-23 | yes |
