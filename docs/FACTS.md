...
| School deadlines are meant to arrive by **notification email** — an Outlook forward to `school@onesid.ca`, received by the gateway's `email()` handler. **The handler is wired in code but has never received a single email:** `d2l_email_messages` and `d2l_email_failure_state` are both empty in production, so not even a malformed message has reached it. School mail does not reach Jarvis today. The likely cause is the Cloudflare routing rule for `school@onesid.ca` still pointing at Gmail — **unverified**; only the dashboard settles it | Queried both tables in production | 2026-09-21 | yes |
...
| **The school account has no MFA.** Signing in is a password and a click; a stale session logs you out and needs a second attempt. **This retracts an unverified prohibition in two runbooks** (`brightspace-calendar-feed.md`, `d2l-notification-email.md`) that forbade Jarvis holding a school password on the stated grounds that it could not work. Nothing recorded MFA before this date — the obstacle was assumed, not measured | Sid, 2026-09-21 | 2026-09-21 | yes |
...
| **Voice cannot carry tools as a matter of type, not wiring.** `ModelAdapterStreamInput` has no `tools` field; tools exist only on `ModelAgentCompletionInput`, consumed by `DeepSeekAgentProvider`. Composing tools into a call is a type change, not a composition change | Read `model/model-adapter.ts` and `providers/provider-types.ts` | 2026-09-20 | yes |
| **`DEEPSEEK_MODEL` is bound as a production secret and its value cannot be read back.** Secrets are write-only. The code default was `deepseek-v4-pro` and is now `deepseek-flash`; if the binding says `v4-pro`, Jarvis runs a model its owner did not choose at roughly seven times the price. **Only Sid can check it, in the Cloudflare dashboard** | `wrangler secret list` shows the name, never the value | 2026-09-20 | **unknown — confirm with Sid** |
| **The gateway heartbeat works as of 2026-09-20.** Every cron before that deploy logged `status 404` and the watchdog had never once recorded the gateway. `component_liveness` now holds `cloud-gateway`. The fix needed a redeploy to prove | Queried `component_liveness` directly after deploying Worker `74f2a003` | 2026-09-20 | yes |
| **Migration `0038` is applied and the gateway is deployed.** D1 is at `0038`; Worker `74f2a003`, watchdog `c940f9b7`. The memory rebuild is live rather than merged-and-inert | Applied and deployed 2026-09-20, verified by querying `d1_migrations` and `sqlite_schema` | 2026-09-20 | yes |
...
| Calling's remaining gap **as of `ca88bf4`**, before #135: the voice path composes its **own** `DefaultConversationService` (`src/voice/production-runtime.ts`) with a bare `DeepSeekModelAdapter` and **zero tools** | `production-runtime.ts` and `src/index.ts` read at `ca88bf4`; `src/voice` contains no tool, function-call or tool-call reference | 2026-09-19 | superseded by the row above |
| **Corrected 2026-09-21 — an earlier version of this row was itself wrong, and the mistake is worth keeping.** I wrote that voice and Telegram *"both use `D1ContextRetriever`… the composition site is what is duplicated"*, based on the voice path alone. **Telegram does not use it.** `index.ts` composes `TelegramMemoryRetriever`; only `production-runtime.ts` composes `D1ContextRetriever`. The two read **different stores** — `memory_item_fts`/`memory_item_versions` versus `memory_fact_projection_fts`/`events`. The deeper difference, and the sharper statement of the gap: `D1ContextRetriever implements ContextRetriever` **only**, while `TelegramMemoryRetriever` also implements **`TelegramMemoryTargetFinder`**. So voice cannot name a specific memory to act on — it has no `findControlTargets` — which is why the pin path has nothing to reach | `src/index.ts:233` and `src/voice/production-runtime.ts:110` at `0611803`; the two `implements` clauses read directly | 2026-09-21 | yes |
