...
| **Who builds and who reviews is not per-milestone.** **DeepSeek builds** every milestone; **Claude reviews** every PR; **both** do the deep final review of everything at milestone exit. The reverse vendor row this replaced (`R4`, Claude building) is gone — St. Remy is out of scope for this repository | Sid stated the arrangement directly; `docs/BUILDING.md` carries it and named `GPT-5.6 Sol`, which this row corrected in the same change | 2026-09-20 | yes |
| The authoritative roadmap is **`docs/plan/2026-09-19-jarvis-roadmap.md`**, which has **seven phases and no `R` numbers**. The `2026-09-03` roadmap carries a `SUPERSEDED` banner telling sessions not to open work against an `R` number from it | Read at `53c4d56`; the new file's only `R`-number occurrence is prose in the supersession note | 2026-09-20 | yes |
