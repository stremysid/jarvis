...
## Owner deadline proof contract (#166 review rounds 1–2)
...
| `resolveDate` | Next weekday and a bare weekday naming today are ambiguous. Both supported dates are returned with `deadline_ambiguous_date`; no candidate is stored by a code convention. Other supported forms remain ISO, English month/day or day/month, explicit this weekday, today/tomorrow, next week optionally with weekday, and ordinal day. | Ask Sid which candidate date he means. The model proposes the resolution; code proves it or refuses. The retained nearest-occurrence rule for omitted year/ordinal and the unconfirmed bare-next-week upper bound remain **judgment findings**, not owner-approved exceptions; disclosure alone does not cure them. |
| `proveDeadlineDue` bare-clock branch | A clock without a date (`3pm`, `at 3pm`, `tonight at 11:59pm`) uses the durable message's local date in the owner zone, as Sid explicitly requested in round 2. Already-passed bare/weekday clocks return `deadline_time_already_passed`, never an automatic tomorrow. | The receipt names the proven local time; refusal asks for the intended date. Explicit historical dates remain possible for reporting past deadlines. |
...
| `matchingDeadline` / `recordDeadline` | Only case/whitespace share a principal-scoped identity. Chem/Chemistry uses the existing uncertain-prefix clarification path, not a semantic alias. The literal course and title must precede the due phrase in one sentence. | `deadline_record` asks which assignment is intended when a prefix/punctuation collision occurs; it refuses another sentence's course even if the title/date match. Platform sources still may duplicate owner-reported rows. |
