...
| 14 | `MAX_REPLY_REFERENCES` / `replyReferences` (`src/agent/owner-agent-core.ts`), found in [#200](https://github.com/stremysid/jarvis/pull/200) | Which memory references one reply carries. `MAX_REPLY_REFERENCES = 8` mirrors the store's `MAX_REFERENCED_ITEMS = 8` (the store replaces rather than appends), but the *selection* inside that bound is code's: `replyReferences` keeps the **8 most recent** items the turn touched and drops earlier ones, so a long chain that searched several topics can end with the reply referring to the last thing it read rather than the item the reply is about. | The cap is a storage bound and may stay; the rule "most recent wins" is a relevance judgment. Either carry every reference the turn touched and let the store's own bound apply, or expose the turn's references to the model and let it choose which the reply is about. |
