...
## Owner deadline proof contract (#166 review rounds 1–3)
...
| `proveDeadlineDue` bare-clock branch | A clock without a date (`3pm`, `at 3pm`, `tonight at 11:59pm`) uses the durable message's local date in the owner zone under the round-2 review contract. Already-passed bare/weekday/today clocks return `deadline_time_already_passed`, never an automatic tomorrow. A tomorrow clock that is also still ahead on the message date, and a tonight a.m. clock, return `deadline_ambiguous_date` naming both dates instead of using an invented late-night cut-off. | The receipt names the proven local time; refusal asks for the intended date. Explicit historical dates remain possible for reporting past deadlines. |
...
| `matchingDeadline` / `recordDeadline` | Only case/whitespace share a principal-scoped identity. Chem/Chemistry uses the existing uncertain-prefix clarification path, not a semantic alias. The literal course and title must precede the due phrase in one clause, with no intervening date or clock. | `deadline_record` asks which assignment is intended when a prefix/punctuation collision occurs; it refuses another sentence's course or title and any due phrase separated from the title by a comma, `and`, or another date/clock. Platform sources still may duplicate owner-reported rows. |
