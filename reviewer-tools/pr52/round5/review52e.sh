...
node -e 'const s=JSON.parse(require("fs").readFileSync(process.argv[1],"utf8"));console.log("trigger mutations:",s.mutations.length-1)' "$OUT/mut52e-triggers.json"
...
echo "trigger timeouts: $(grep -a -ci 'timed out' "$OUT/run52etrig.txt")"
awk '/^== /{if(id!="")print id" | "n" failed | "b; id=$2; n=0; b=""} /^ +× /{n++; b=b"["substr($0,index($0,"×")+2,60)"] "} END{print id" | "n" failed | "b}' "$OUT/run52etrig.txt"
grep -a -E "MATCHED|no test summary" "$OUT/run52etrig.txt" | head
