...
for step in lint typecheck test; do pnpm.cmd $step > "$OUT/53-$step.txt" 2>&1; echo "$step exit $?"; done
grep -a -E "Test Files|Tests +[0-9]" "$OUT/53-test.txt" | tail -2; echo "timeouts: $(grep -a -ci 'timed out' "$OUT/53-test.txt")"
grep -a -E '^\s*(×|FAIL )' "$OUT/53-test.txt" | head -8
...
node "$ME/tools/gen-trig.mjs" "$R/$SQL" "$OUT/mut53-triggers.json" "$H" "$R" "$SQL" "$TESTS"
node -e 'const s=JSON.parse(require("fs").readFileSync(process.argv[1],"utf8"));console.log("trigger mutations:",s.mutations.length-1)' "$OUT/mut53-triggers.json"
echo "clean before triggers=$([ -z "$(git status --porcelain)" ] && echo yes || echo NO)"
cd "$ME/tools" && node mutrun.mjs "$OUT/mut53-triggers.json" > "$OUT/run53trig.txt" 2>&1
sed -n '/=== summary/,$p' "$OUT/run53trig.txt" | grep -v DEP0190 | grep -v trace-deprecation
echo "trigger timeouts: $(grep -a -ci 'timed out' "$OUT/run53trig.txt")"
awk '/^== /{if(id!="")print id" | "n" failed | "b; id=$2; n=0; b=""} /^ +× /{n++; b=b"["substr($0,index($0,"×")+2,60)"] "} END{print id" | "n" failed | "b}' "$OUT/run53trig.txt"
grep -a -E "MATCHED|no test summary" "$OUT/run53trig.txt" | head
