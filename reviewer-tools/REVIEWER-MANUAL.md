...
**Migration numbers** are reserved by a file on some branch, not on main — and not only on a branch with a PR. Before accepting a number, run `reviewer-tools/migration-numbers.ps1`: it enumerates every head on origin with `git ls-remote` and reads each one's migrations with `git ls-tree` (no checkout), then reports any number contested by more than one pending branch, any branch that rewrote a migration main already applies, and the next genuinely free number. Findings a branch cannot act on — a number main already took while that branch was forked, or a superseded copy of an unapplied migration — are printed and not counted, so the verdict stays readable. It exits 2 rather than answering when a branch could not be compared. Collisions happened three times, the first two found by eye: 0018, 0035, then 0036 — and the third is the reason the enumeration is `git ls-remote` and not `gh pr list`: one of the two branches holding 0036 had been pushed and had no PR, and a PR-only scan called that repository clean. Still check AGENT_LOG for a reservation that has no file yet. `reviewer-tools/migration-numbers.verify.ps1` proves each verdict is still reachable.
