...
console.log("real repository digest rows after Sid said", JSON.stringify(msg), rows.map((r) => `${r.university} ${r.programName}: ${r.label} [${r.status}] due=${r.dueDate ?? r.dueAt ?? "none"}`));
