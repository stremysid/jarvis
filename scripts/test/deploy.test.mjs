...
/**
 * A throwaway checkout holding one deploy script and a fake wrangler, as a real
 * repository with a real remote: a clean checkout of origin/main, which is the
 * only thing a publish may ship.
 */
function withCheckout(script, app, body) {
...
`);
    ...
    body({ root, origin, config, capture, git, run, checkArguments });
  } finally {
...
for (const [script, app] of [['deploy.ps1', 'cloud-gateway'], ['deploy-watchdog.ps1', 'watchdog']]) {
  test(`${script} preserves production targeting across the real PowerShell native boundary`, () => {
    withCheckout(script, app, ({ capture, run, checkArguments }) => {
  ...
  test(`${script} refuses to publish anything but a clean checkout of origin/main, and names what it found`, () => {
    withCheckout(script, app, ({ root, origin, config, capture, git, run, checkArguments }) => {
      // Each refusal must stop before wrangler runs. PowerShell colours its error
      // record and wraps it at the console width, so the message is flattened
      // back to one line before it is read.
      ...
      // must fetch to see it, because the local origin/main ref still matches HEAD.
      git(root, 'push', '-q', 'origin', 'main');
